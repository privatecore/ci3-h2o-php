...
class CoreFilters extends FilterCollection
{
	...
	static function urlencode($data)
	{
		if (is_array($data))
		{
			...
			{
				$result .= $name.'='.urlencode($value).'&'.$sqlstring;
			}

			$sqlstring = substr($result, 0, strlen($result)-1);
...
class StringFilters extends FilterCollection
{
	...
	static function capfirst($string)
	{
		...
		return strtoupper($string{0}). utf8_substr($string, 1, utf8_strlen($string));
	}
	...
	{
		return (utf8_strlen($string) > $max ? utf8_substr($string, 0, $max).$ends : $string);
	}
	...
	static function limitwords($text, $limit = 50, $ends = '...')
	{
		if (utf8_strlen($text) > $limit)
		{
			...
			if (isset($pos[$limit]))
			{
				$text = utf8_substr($text, 0, $pos[$limit]) . $ends;
			}
		}
...
h2o::addFilter(array('md5', 'sha1', 'numberformat'=>'number_format', 'wordwrap', 'trim', 'upper' => 'utf8_strtoupper', 'lower' => 'utf8_strtolower'));
