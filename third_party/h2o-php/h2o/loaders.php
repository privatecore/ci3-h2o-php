...
class H2o_File_Loader extends H2o_Loader
{
	...
	{
		if (isset($options['cache_type']) && $options['cache_type'])
		{
	...
	function read_cache($filename)
	{
		...
		//$cache = basename($filename, substr(strrchr($filename, '.'), 0)) . '_' . md5($filename);
		$cache = basename($filename, substr(strrchr($filename, '.'), 0));
